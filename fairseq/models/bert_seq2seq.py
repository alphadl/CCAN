...
from fairseq.modules import (
    AdaptiveSoftmax, CharacterTokenEmbedder, MultiheadAttention, ContextAwareMultiheadAttention,
...
class Transformer_nonautoregressive(FairseqModel):
    ...
    def add_args(parser):
        ...
        # args for training localness cross attention
        parser.add_argument('--win-size', default=3, type=int, metavar='N',
                            help='the window size of local pattern')
        parser.add_argument('--decoder-layers-to-apply-local', default=None,
                            help='which layers to *apply localness modeling*')
...
class SelfTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_scale=None, no_encoder_attn=False, left_pad=False,
        ...
        if args.decoder_layers_to_apply_local:
            decoder_layers_to_apply_local = [int(l)-1 for l in args.decoder_layers_to_apply_local.split(",")]

        self.layers = nn.ModuleList([])
        if args.decoder_layers_to_apply_local:
            for _ in range(args.decoder_layers):
                if _ in decoder_layers_to_apply_local:
                    self.layers.extend([TransformerDecoderLayer(args, no_encoder_attn, local_modeling=True)])
                else:
                    self.layers.extend([TransformerDecoderLayer(args, no_encoder_attn)])
        else:
            self.layers.extend([
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, local_modeling=False):
        super().__init__()
        self.embed_dim = args.decoder_embed_dim
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn = ContextAwareMultiheadAttention(
                self.embed_dim, args.decoder_attention_heads,
                dropout=args.attention_dropout,
